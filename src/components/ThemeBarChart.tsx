        ...
        return (
          <div key={row.theme}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
              ...
              <span style={{ flex: 1, fontSize: 13, fontWeight: isTop ? 700 : 500, color: "var(--charcoal)" }}>
                {getThemeLabel(row.theme)}
              </span>
              <span style={{ fontSize: 13, fontWeight: 700, color: isTop ? color : "var(--grey-mid)", minWidth: 44, textAlign: "right" }}>
                {row.pct}%
              </span>
              <span style={{ fontSize: 11, color: "var(--grey-mid)", minWidth: 48, textAlign: "right" }}>
                n={row.count}
              </span>
            </div>
            <div style={{ height: isTop ? 7 : 5, background: "var(--bg)", borderRadius: 3, overflow: "hidden" }}>
              <div style={{
                width: `${(row.pct / maxPct) * 100}%`, height: "100%",
